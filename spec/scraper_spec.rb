...
describe Scraper do
  ...
  describe '.initialize' do
    it 'Check url builder in initialize method' do
      expect(@scraper.instance_variable_get(:@url)).to eq('https://katcr.to/usearch/rick%20and%20morty/')
    end
  ...
  describe '.results_counter' do
    it 'Checks number of results available' do
      expect(@scraper.results_counter).to eq(645)
    end
  ...
  describe '.extract_torrent' do
    it 'Displays name of torrent of list item' do
      ...
      first_torrent = []
      torrent_list[1].each { |item| first_torrent << item }
      expect(first_torrent[1]).to eq("Rick and Morty S04E03 1080p WEBRip x264-TBS TGx ⭐ \n")
    end
  ...
  describe '.extract_torrent' do
    ...
    it 'Checks that torrents are not getting duplicated' do
      ...
      first_torrent = []
      torrent_list[1].each { |item| first_torrent << item }
      expect(first_torrent[6]).to eq("HD \n")
    end
  end
end
